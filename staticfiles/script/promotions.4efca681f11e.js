    ...
    try {
        ...
        setTimeout(() => {
            if (typeof $ !== 'undefined' && typeof $.fn.select2 !== 'undefined') {
                ...
                console.log('[PROMOTIONS] Select2 non disponible, utilisation des selects natifs');
            }
        }, 500);
...
// Initialiser Select2 sur les selects
function initializeSelect2() {
    try {
        // Vérifier si jQuery et Select2 sont disponibles
        if (typeof $ === 'undefined' || typeof $.fn.select2 === 'undefined') {
            console.warn('[PROMOTIONS] jQuery ou Select2 non disponible, utilisation des selects natifs');
            return;
        }

        // Détruire les instances existantes si elles existent
        const $produitSelect = $('#promo-produit');
        const $categorieSelect = $('#promo-categorie');

        if (!$produitSelect.length || !$categorieSelect.length) {
            console.warn('[PROMOTIONS] Selects non trouvés dans le DOM');
            return;
        }

        if ($produitSelect.data('select2')) {
            $produitSelect.select2('destroy');
        }
        if ($categorieSelect.data('select2')) {
            $categorieSelect.select2('destroy');
        }

        // Initialiser Select2 avec options personnalisées
        $produitSelect.select2({
        ...
        // Les event listeners natifs géreront les changements
        // Select2 déclenche les événements 'change' natifs, donc pas besoin de dupliquer les listeners

        console.log('[PROMOTIONS] Select2 initialisé avec succès');
    } catch (error) {
        console.error('[PROMOTIONS] Erreur lors de l\'initialisation de Select2:', error);
        console.log('[PROMOTIONS] Utilisation des selects natifs');
    }
}
...
async function editPromotion(id) {
    try {
        ...
        // Produit/Catégorie
        if (typeof $ !== 'undefined' && typeof $.fn.select2 !== 'undefined') {
            $('#promo-produit').val(promo.produit || '').trigger('change');
            $('#promo-categorie').val(promo.categorie || '').trigger('change');
        } else {

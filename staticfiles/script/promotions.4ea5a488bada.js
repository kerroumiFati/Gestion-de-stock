...
async function loadProduits() {
    try {
        ...
        PromoConfig.cache.produits = await response.json();
        ...
        PromoConfig.cache.produits.forEach(p => {
            ...
            option.value = p.id;
            option.textContent = `${p.reference} - ${p.designation} (${p.prix_formatted || p.prixU + ' €'})`;
...
async function loadCategories() {
    try {
        ...
        PromoConfig.cache.categories = await response.json();
        ...
        PromoConfig.cache.categories.forEach(c => {
            const option = document.createElement('option');
            option.value = c.id;
            option.textContent = c.full_path || c.nom;
            select.appendChild(option);
        });
    } catch (error) {
